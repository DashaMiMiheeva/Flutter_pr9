            ...
            return Column(
              children: [
                ListTile(
                  ...
                  onTap: () => _pickTime(
                      context, state.breakfastTime, context.read<ReminderCubit>().setBreakfastTime),
                ),
                ListTile(
                  title: const Text("Обед"),
                  trailing: Text("${state.lunchTime.hour.toString().padLeft(2,'0')}:${state.lunchTime.minute.toString().padLeft(2,'0')}"),
                  onTap: () => _pickTime(
                      context, state.lunchTime, context.read<ReminderCubit>().setLunchTime),
                ),
                ListTile(
                  ...
                  onTap: () => _pickTime(
                      context, state.dinnerTime, context.read<ReminderCubit>().setDinnerTime),
                ),
              ],
            );
          },
